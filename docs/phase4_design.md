...
レシート（支出/収入）の入力画面を実装する。上部タブで支出・収入を切り替え、日付・メモを入力し、明細をドロワーで追加する。
...
1. API にレシートを `POST`
2. 成功時: 入力フォームを**リセット**（日付→今日、メモ→空、明細→空）
3. **トースト**で「保存しました」を表示（2〜3秒後に自動消失）
4. エラー時: トーストでエラーメッセージを表示

---

## 検証

- カテゴリー API が支出/収入別に正しく返す
- ドロワーで明細追加 → リストに表示
- 日付の ◀ ▶ で1日ずつ変更
- テンキーで金額入力
- 保存 → DB保存 → フォームリセット → トースト表示
